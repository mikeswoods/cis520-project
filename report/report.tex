...
The dataset itself consisted of 25,000 training reviews, 5,000 quiz reviews (no labels provided), a vocabulary lexicon of 56,835 words, and associated metadata for each review. Each review was presented as a ``bag of words'' count, in which each review was decomposed into a vector with entries denoting the presence or absence of the $i$-th word in the vocabulary lexicon, and if present, how many times that word appeared. The lexicon itself was constructed from the tokenized text of all training and quiz examples, with punctuation removed and all words normalized to lowercase. The metadata of each review consisted of a unique user ID, business ID, number of ``cool'' / ``funny'' / ``useful'' tags the review received, and the full text of the review as a list of tokens in sequential order without any preprocessing applied.

In order to judge the effectiveness of the algorithms, the final assessment of predictor performance consisted of a dataset of 10,000 reviews, with corresponding labels that the final, trained learners would be asked to predict. The top teams with the learners that produced the lowest RMSE on the test dataset would be declared the winners.

\section{Tools Used}
\matlab, specifically the Statistics Toolbox, was used to create the bulk of our predictive engine. Outside of the \matlab~standard library, we leveraged the \liblinear~package as the underpinning for the implementation of several of our learners. This was due to \liblinear's excellent support for working with large matrices, specifically sparse ones. Outside of \matlab, we used Python to process the full text of the reviews in order to generate bigrams. The decision to use Python was made, due in large part, to the cumbersome nature of text processing in \matlab, and the availability of the SciPy package to read and write \texttt{.mat} files. Git, along with Github, was used for version control.

\section{Structure}
Based on the results of the Netflix prize, and the intuition that the combination of multiple opinions are ``better'' than one, from the outset we believed that an \textbf{ensemble of learners} would most likely yield the best results. ``Better'' in the case of this competition meant creating a final predictor that achieved the lowest possible RMSE during testing, while avoiding the effect of overfitting when the same code was run against out-of-sample test reviews.

A number of methods were employed to combine the output ratings from multiple predictors into an ensemble predictor. Specifically, we implemented the following methods of combining ratings:
\begin{enumerate}
\item \textbf{Weighted average, with constant weights.} The ratings were simply averaged together, with each learner having the same importance in the average.

\item \textbf{Weighted average, with weights determined by linear regression.} Predicted ratings were first generated from each learner using the training dataset, then a linear regression was run with the predicted ratings serving as the observation matrix, \textbf{X}, and the actual ratings as the label vector, \textbf{Y}. The resulting weight vector, \textbf{w}, was used to generate a weighted average prediction, with the $i$-th entry of \textbf{w} scaling the vector of predicted ratings produced by the $i$-th learner.
...
Each learner was implemented as a \matlab~package with three functions: \texttt{train}, \texttt{predict}, and test. This provided a simple interface for the training and cross validation code to interact with the learners in a simple and consistent way. 
...
A \nb~classifier was implemented using the \matlab~\texttt{NaiveBayes} class using a multinomial distribution for training. The multinomial distribution was chosen, as a bag-of-tokens model for the underlying data distribution mirrored how the data was stored for each review. With cross validation ($N=10$), the \nb~learner yielded an average RMSE of 1.057 and an average accuracy of 48.69\%. A positive aspect of the \nb~classifier, at least the \matlab~implementation thereof, was a relatively flat train-time-versus-training-set-size relationship; see Figure \ref{fig:timenobs}. Both the logistic regression and SVM learners displayed an approximately-quadratic growth rate in training time duration with number of observations trained. The \nb~implementation was much more constant. Even using the entire training set, class \texttt{nb} provided fast results. Through cross-validation, the \nb~learner proved to be resistant to overfitting at this training set size.

\texttt{nb}, along with \texttt{counts\_svm} and \texttt{counts\_logit\_reg}, composed our final ensemble learner.
...
\begin{figure}[h!]
	...
	\centering
		...
		\caption{RMSE performance of learners versus a 2500-sample holdout set versus relative size of training set (as a fraction of full provided training set, 25,000 observations). Test set size is a constant 0.1 fraction of full training set.}
		\label{fig:rmsenobs}
	\end{subfigure}
	\quad
	\begin{subfigure}{.45\textwidth}
	\centering
		...
		\caption{Training time of learners (in sec) versus a 2500-sample holdout set versus relative size of training set (as a fraction of full provided training set, 25,000 observations). Test set size is a constant 0.1 fraction of full training set.}
		\label{fig:timenobs}
	\end{subfigure}
	\caption{Behavior of learners with increased training set size}
\end{figure}

\subsection{``Unsuccessful'' Component Learners}
\subsubsection{\texttt{eigenwords\_logit\_reg}}
Bigram matrices were generated from the combined training and quiz sets using a customized Python function (using the complete text as found in the supplied metadata). We performed a singular value decomposition (SVD) on the bigram matrices, generating reduced-dimension (ending up with $k$ dimensions) left and right singular vectors (\textbf{U} and \textbf{V}, respectively) of size $V\times k$ for each of $V$ words in the vocabulary. Predictors for each document were the average position (centroid) of the document's words in the reduced-dimension space. This method was patterned off of Foster, Liberman, and Stine (unpublished)\footnote{``Featurizing Text: Converting Text into Predictors for Regression Analysis.'' Dean P. Foster, Mark Liberman, and Robert A. Stine. Unpublished.}.
...
Due to the huge number of dimensions per review in the dataset, dimensionality reduction of some sort was needed in order to train certain classes of learners. For example, \matlab's built-in \texttt{kmeans} function was entirely unusable since we speculate within the function, it converts any sparse matrix passed into it into a full matrix. Any manipulations of the full matrix representation of the training dataset became all but impossible with a commodity piece of hardware like a MacBook. Moreover, we could not use \matlab’s built-in functions for principal component analysis like \texttt{pca} or \texttt{princomp}, as performing mean-centering on the sparse training data matrix converted it to a full matrix, which again, made it intractable to work with. One potential workaround we utilized to a varying degree of success involved simply performing SVD on a sparse matrix itself without mean centering, and taking the top $k + 1$ singular vectors. The first singular vector was discarded, and the rest were used as the top $k$ principal components.

The reduced-dimensionality dataset gave similar performance to the full sparse dataset using logistic regression and SVM methods. The \liblinear~packages we used, however, were able to train models on the full sparse data faster than other, non-sparse-optimized packages like \textsf{LIBSVM} could train on the non-sparse full data. In order to allow us to complete multiple rounds of cross-validation in reasonable time, we decided not to use the reduced-dimension data. %***Maybe some analysis of why this did not perform too well? ***

\subsection{Final Learner}
The final learner used to generate predictions for the assessment test set consisted of \texttt{counts\_logit\_reg}, \texttt{nb}, \texttt{counts\_svm}. The resulting predicted rating from the three learners were combined using a weighted average with the following weights: $[0.417, 0.368, 0.172]$ (respectively). The weights were determined experimentally through cross-validation ($N=10$), and observing that that weights produced via linear regression consistently produced the lowest, or second lowest RMSE for each round of cross-validation.
...
We performed reasonably well with 3 relatively simple classifiers: \nb, logistic regression, and support vector machines. Our performance was also consistent across cross-validation, against the quiz set, and against the test set, which shows we did not have any trouble with overfitting in the final ensemble.
...
Finally, improving the n-gram creation to skip common stop words, or stemming the words before generating bigram matrices, may have improved the performance of the eigenword-based approaches, combining similar words like ``love'' and ``loved'' which effectively function the same, contextually. Stemming words in the bag-of-words counts matrix would have reduced dimensionality and increased counts (i.e., combined columns), possibly leading to better learners on the counts matrix.
