...
%    variable used to create the Naive Bayes classifier.
    ...
    methods(Access = 'public', Static)
        ...
        function   logCondPDF=getlogCondPDF(obj,test, nb_mat,handleNaNs)
            nTest= size(test,1);
            ...
            if  isscalar(obj.Dist) && strcmp(obj.Dist,'mn')
                ...
                if any(obj.MVMNFS)
                    ...
                    if any(testUnseen)
                        ...
                        warning(message('stats:NaiveBayes2:BadDataforMVMN'));
                        test(testUnseen,:)=[];
                        tempIdx (testUnseen,:)=[];
                    end
                else
     ...
     methods(Hidden, Static)	 
         function a = empty(varargin)	 
             error(message('stats:NaiveBayes2:NoEmptyAllowed', upper(mfilename))); 
         end   
     end
...
error(message('stats:NaiveBayes2:NoCatAllowed',upper(mfilename))); 
end
