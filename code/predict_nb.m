function gpre = predict_nb(obj,test,nb_mat,varargin)
...
if nargin < 2
    error(message('stats:NaiveBayes2:predict:TooFewInputs'));
end

if ~isnumeric(test) 
    error(message('stats:NaiveBayes2:predict:TestBadType'));
end

if ~isreal(test)
    error(message('stats:NaiveBayes2:predict:TestComplexType'));
end

if size(test,2)~= obj.NDims
      error(message('stats:NaiveBayes2:BadTestSize', obj.NDims));
end
...
if isscalar(obj.Dist) && strcmp(obj.Dist,'mn')
    ...
    if any(testBad)
        warning(message('stats:NaiveBayes2:predict:BadDataforMN'));
    ...
    try
      gpre= dfswitchyard('statinsertnan',isGpreNaN,gpre);
        ...
        if ~isequal(ME.identifier,'stats:statinsertnan:InputTypeIncorrect')
                 ...
                 error(message('stats:NaiveBayes2:predict:logicalwithNaN'));
         end
    end
end
